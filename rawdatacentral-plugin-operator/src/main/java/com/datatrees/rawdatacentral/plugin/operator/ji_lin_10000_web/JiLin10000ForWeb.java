...
public class JiLin10000ForWeb implements OperatorPluginService {
    ...
    private HttpResult<Map<String, Object>> submitForLogin(OperatorParam param) {
        ...
        try {
            ...
            templateUrl = "http://login.189.cn/login";
            templateData = "Account=" + param.getMobile() + "&UType=201&ProvinceID=09&AreaCode=&CityNo=&RandomFlag=0&Password=" +
                    URLEncoder.encode(encryptPassword, "UTF-8") + "&Captcha=";
            referer = "http://login.189.cn/login";
            response = TaskHttpClient.create(param, RequestType.POST, "ji_lin_10000_web_006").setFullUrl(templateUrl).setRequestBody(templateData)
                    .setReferer(referer).invoke();
            pageContent = response.getPageContent();
            if (StringUtils.isBlank(pageContent)) {
    ...
    private HttpResult<String> refeshPicCodeForUserInfo(OperatorParam param) {
        ...
        try {
            ...
            response = TaskHttpClient.create(param, RequestType.POST, "ji_lin_10000_web_009").setFullUrl(templateUrl).setRequestBody(templateData).addHeader("Accept","application/json, text/javascript, */*; q=0.01")
                    .setReferer(referer).invoke();
            String pageContent = response.getPageContent();
