package com.datatrees.rawdatacentral.plugin.common._126.com;
 ...
 * Date: 2018/2/26
 */
public class _126MailPlugin implements CommonPluginService, QRPluginService {

    private static final Logger logger = LoggerFactory.getLogger(_126MailPlugin.class);
    ...
    public HttpResult<Object> submit(CommonPluginParam param) {
        ...
        try {
            ...
            BeanFactoryUtils.getBean(ThreadPoolService.class).getMailLoginExecutors().submit(new Runnable() {
                @Override
                public void run() {
                    String currentUrl = "https://mail.126.com/";
                    ...
                    try {
                        ...
                        if (isLoginSuccess(driver, param)) {
                            ...
                            currentUrl = "https://mail.126.com/entry/cgi/ntesdoor?from=smart";
                            driver.get(currentUrl);
                            TimeUnit.SECONDS.sleep(3);
                            currentUrl = driver.getCurrentUrl();
                            LoginMessage loginMessage = new LoginMessage();
                            loginMessage.setTaskId(taskId);
                            loginMessage.setWebsiteName(GroupEnum.MAIL_126.getWebsiteName());
            ...
            BeanFactoryUtils.getBean(ThreadPoolService.class).getMailLoginExecutors().submit(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        Response response = TaskHttpClient.create(param, RequestType.GET).setUrl("https://reg.163.com/services/getqrcodeid?product=mail126&usage=web").invoke();
                        ...
                        response = TaskHttpClient.create(param, RequestType.GET).setFullUrl("https://reg.163.com/services/getUrlQrcode?uuid={}&size=170", uuid).invoke();
                        ...
                        while ((StringUtils.equals(qrStatus, QRStatus.WAITING) || StringUtils.equals(qrStatus, QRStatus.SCANNED)) && !expire && lastLoginProcessId) {
                            TimeUnit.SECONDS.sleep(3);
                        ...
                        logger.warn("thread will close taskId={},qrStatus={},expire={},isLastLoginProcessId={}", taskId, qrStatus, expire, lastLoginProcessId);

                        if (StringUtils.equals(qrStatus, QRStatus.SUCCESS) && TaskUtils.isLastLoginProcessId(taskId, processId)) {
                            response = TaskHttpClient.create(param, RequestType.GET).setFullUrl("https://reg.163.com/services/qrcodeauth?uuid={}&product=mail126", uuid).invoke();
                            String ticket = response.getPageContentForJSON().getString("ticket");

                            String url = "https://reg.163.com/services/ticketlogin";
                            String body = "noRedirect=1&product=mail126&ticket="+ticket+"&url=https://mail.126.com/entry/cgi/ntesdoor?allssl=true&df=mail126_mailmaster&from=web&language=-1&net" +
    ...
    private String getScandStatus(CommonPluginParam param, String uuid) {
        ...
        try {
            Response response = TaskHttpClient.create(param, RequestType.GET).setFullUrl("https://q.reg.163.com/services/ngxqrcodeauthstatus?uuid={}&product=mail126", uuid).invoke();
    ...
    private boolean isLoginSuccess(RemoteWebDriver driver, CommonPluginParam param) {
        ...
        return StringUtils.startsWith(currentUrl, "https://mail.126.com/js6/main.jsp?sid=");
    }

    private boolean isShowError(RemoteWebDriver driver, CommonPluginParam param) {
        try {
            String currentUrl = driver.getCurrentUrl();
            if (StringUtils.equals(currentUrl, "https://mail.126.com/")) {
