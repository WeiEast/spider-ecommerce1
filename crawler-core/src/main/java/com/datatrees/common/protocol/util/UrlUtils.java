...
public class UrlUtils {
    ...
    private static final Pattern URL_NAME_PATTERN = PatternUtils
            .compile("(https?://)?(\\w+\\.)?(([^/\\s]+)\\.\\w+)(/.*)?");
    private static final Pattern URL_PATTERN      = PatternUtils
            .compile("^https?(:|&#58;)//([-\\w]+\\.)+[\\w-]+(:\\d+)?(/[^'<>,、，\\s\\\"]*)?");
    ...
    public static URL encodeUrl(final URL url, final boolean minimalQueryEncoding) {
        ...
        if ("javascript".equalsIgnoreCase(p) || "about".equalsIgnoreCase(p)
        ...
        try {
            ...
            return createNewUrl(url.getProtocol(), url.getHost(), url.getPort(), path, anchor,
                    query);
    ...
    public static URL getUrlWithNewProtocol(final URL u, final String newProtocol)
        ...
        return createNewUrl(newProtocol, u.getHost(), u.getPort(), u.getPath(), u.getRef(),
                u.getQuery());
    }
    ...
    public static URL getUrlWithNewHost(final URL u, final String newHost)
            throws MalformedURLException {
        return createNewUrl(u.getProtocol(), newHost, u.getPort(), u.getPath(), u.getRef(),
                u.getQuery());
    }
     ...
     * @throws MalformedURLException if there is a problem creating the new URL
     */
    public static URL getUrlWithNewPort(final URL u, final int newPort)
            throws MalformedURLException {
        return createNewUrl(u.getProtocol(), u.getHost(), newPort, u.getPath(), u.getRef(),
                u.getQuery());
    }
    ...
    public static URL getUrlWithNewPath(final URL u, final String newPath)
            throws MalformedURLException {
        return createNewUrl(u.getProtocol(), u.getHost(), u.getPort(), newPath, u.getRef(),
                u.getQuery());
    }
    ...
    public static URL getUrlWithNewRef(final URL u, final String newRef)
            throws MalformedURLException {
        return createNewUrl(u.getProtocol(), u.getHost(), u.getPort(), u.getPath(), newRef,
                u.getQuery());
    }
    ...
    public static URL getUrlWithNewQuery(final URL u, final String newQuery)
            throws MalformedURLException {
        return createNewUrl(u.getProtocol(), u.getHost(), u.getPort(), u.getPath(), u.getRef(),
                newQuery);
    }
    ...
    private static Url resolveUrl(final Url baseUrl, final String relativeUrl) {
        ...
        while ((pathSegmentIndex = path.indexOf("/./")) >= 0) {
            path =
                    path.substring(0, pathSegmentIndex + 1).concat(
        ...
        while ((pathSegmentIndex = path.indexOf("/../")) > 0) {
            ...
            if (!pathSegment.substring(slashIndex).equals("..")) {
                path =
                        path.substring(0, slashIndex + 1).concat(
