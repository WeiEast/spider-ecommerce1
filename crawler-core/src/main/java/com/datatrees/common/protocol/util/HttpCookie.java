...
public final class HttpCookie implements Cloneable {
     ...
     * <p>
     * By default, cookies are created according to the RFC 2965 cookie specification. The version
     * can be changed with the <code>setVersion</code> method.
     * @param name  a <code>String</code> specifying the name of the cookie
     * @param value a <code>String</code> specifying the value of the cookie
     * @exception IllegalArgumentException if the cookie name contains illegal characters or it is one
     *                                     of the tokens reserved for use by the cookie protocol
     * @exception NullPointerException     if <tt>name</tt> is <tt>null</tt>
     * @see #setValue
     * @see #setVersion
     */
    ...
    /**
     * Constructs cookies from set-cookie or set-cookie2 header string. RFC 2965 section 3.2.2
     * set-cookie2 syntax indicates that one header line may contain more than one cookie
     * definitions, so this is a static utility method instead of another constructor.
     * @param header a <tt>String</tt> specifying the set-cookie header. The header should start
     *               with "set-cookie", or "set-cookie2" token; or it should have no leading token at all.
     * @return a List of cookie parsed from header line string
     * @exception IllegalArgumentException if header string violates the cookie specification's syntax,
     *                                     or the cookie name contains llegal characters, or the cookie name is one of the
     *                                     tokens reserved for use by the cookie protocol
     * @exception NullPointerException     if the header string is <tt>null</tt>
     */
    ...
    private static List<HttpCookie> parse(String header, boolean retainHeader) {
        return parse(header, retainHeader, false);
    }

    public static List<HttpCookie> parse(String header, boolean retainHeader, boolean retainQuote) {
     ...
     * <p>
     * This concept is described in the cookie specification. To understand the concept, some
     * terminologies need to be defined first: <blockquote> effective host name = hostname if host
     * name contains dot<br>
     * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&
     ...
     * <li>A is a HDN string and has the form NB, where N is a non-empty name string, B has the form
     * .B', and B' is a HDN string. (So, x.y.com domain-matches .Y.com but not Y.com.)</li>
     * </ul>
     * </blockquote>
     * <p>
     * A host isn't in a domain (RFC 2965 sec. 3.3.2) if: <blockquote>
     * <ul>
     * <li>The value for the Domain attribute contains no embedded dots, and the value is not
     * .local.</li>
     * <li>The effective host name that derives from the request-host does not domain-match the
     * Domain attribute.</li>
     * <li>The request-host is a HDN (not IP address) and has the form HD, where D is the value of
     * the Domain attribute, and H is a string that contains one or more dots.</li>
     * </ul>
     * </blockquote>
     * <p>
     * Examples: <blockquote>
     * <ul>
     * <li>A Set-Cookie2 from request-host y.x.foo.com for Domain=.foo.com would be rejected,
     * because H is y.x and contains a dot.</li>
     * <li>A Set-Cookie2 from request-host x.foo.com for Domain=.foo.com would be accepted.</li>
     * <li>A Set-Cookie2 with Domain=.com or Domain=.com., will always be rejected, because there is
     * no embedded dot.</li>
     * <li>A Set-Cookie2 with Domain=ajax.com will be accepted, and the value for Domain will be
     * taken to be .ajax.com, because a dot gets prepended to the value.</li>
     * <li>A Set-Cookie2 from request-host example for Domain=.local will be accepted, because the
     * effective host name for the request- host is example.local, and example.local domain-matches
     * .local.</li>
     * </ul>
     * </blockquote>
    ...
    private static HttpCookie parseInternal(String header, boolean retainHeader, boolean retainQuote) {
        ...
        try {
            ...
            if (index != -1) {
                ...
                value = retainQuote ? value : stripOffSurroundingQuote(value);
                if (retainHeader) cookie = new HttpCookie(name, value, header);
                else cookie = new HttpCookie(name, value);
            } else {
    ...
    /*
     * Split cookie header string according to rfc 2965: 1) split where it is a comma; 2) but not
     * the comma surrounding by double-quotes, which is the comma inside port list or embeded URIs.
     *
    ...
    private static List<String> splitMultiCookies(String header) {
        ...
        for (p = 0, q = 0; p < header.length(); p++) {
            ...
            if (c == '"') quoteCount++;
            if (c == ',' && (quoteCount % 2 == 0)) { // it is comma and not surrounding by
                // double-quotes
    ...
    /**
     * Returns the comment url describing the purpose of this cookie, or <code>null</code> if the
    ...
    /**
     * Specifies a comment url that describes a cookie's purpose. The comment url is useful if the
     * browser presents the cookie to the user. Comment url is RFC 2965 only.
    ...
    /**
     * Specify whether user agent should discard the cookie unconditionally. This is RFC 2965 only
     * attribute.
    ...
    /**
     * Specify the portlist of the cookie, which restricts the port(s) to which a cookie may be sent
     * back in a Cookie header.
     * @param ports a <tt>String</tt> specify the port list, which is comma seperated series of
     *              digits
     * @see #getPortlist
     */
     ...
     * <p>
     * The form of the domain name is specified by RFC 2965. A domain name begins with a dot (
     ...
     * them.
     * @param pattern a <code>String</code> containing the domain name within which this cookie is
     ...
     *               cookie should be discarded immediately; otherwise, the cookie's max age is
     *               unspecified.
     * @see #getMaxAge
     */
     ...
     * <p>
     * Consult RFC 2965 (available on the Internet) for more information on setting path names for
     * cookies.
    ...
    /**
     * Returns <code>true</code> if sending this cookie should be restricted to a secure protocol,
     * or <code>false</code> if the it can be sent using any protocol.
     * @return <code>false</code> if the cookie can be sent over any standard protocol; otherwise,
     * <code>true</code>
     * @see #setSecure
     */
    ...
    /**
     * Indicates whether the cookie should only be sent using a secure protocol, such as HTTPS or
     * SSL.
     * <p>
     * The default value is <code>false</code>.
     * @param flag If <code>true</code>, the cookie can only be sent over a secure protocol like
     *             https. If <code>false</code>, it can be sent over any protocol.
     * @see #getSecure
     */
     ...
     *
     * @return <code>true</code> if the <code>String</code> is a token; <code>false</code> if it is
     * not
     */
     ...
     * 2965/2109, and version 0 complies with the original cookie specification drafted by Netscape.
     ...
     * the original Netscape cookie specification. Version 1 complies with RFC 2965/2109.
     ...
     * @exception IllegalArgumentException if <tt>v</tt> is neither 0 nor 1
     * @see #getVersion
     */
    ...
    /**
     * Returns {@code true} if this cookie contains the <i>HttpOnly</i> attribute. This means that
    ...
    /**
     * Indicates whether the cookie should be considered HTTP Only. If set to {@code true} it means
     * the cookie should not be accessible to scripting engines like javascript.
     * @param httpOnly if {@code true} make the cookie HTTP only, i.e. only visible as part of an
     *                 HTTP request.
     * @see #isHttpOnly()
     */
    ...
    /**
     * Constructs a cookie header string representation of this cookie, which is in the format
     * defined by corresponding cookie specification, but without the leading "Cookie:" token.
     ...
     * <p>
     * The result is <tt>true</tt> only if two cookies come from same domain (case-insensitive),
    ...
    /**
     * Return hash code of this http cookie. The result is the sum of hash code value of three
     * significant components of this cookie: name, domain, and path. That is, the hash code is the
     * value of the expression: <blockquote> getName().toLowerCase().hashCode()<br>
    ...
    /*
     * Returns the original header this cookie was consructed from, if it was constructed by parsing
     * a header, otherwise null.
     */
    ...
    /*
     * Constructs a string representation of this cookie. The string format is as Netscape spec, but
     * without leading "Cookie:" token.
     */
    ...
    /*
     * Constructs a string representation of this cookie. The string format is as RFC 2965/2109, but
     * without leading "Cookie:" token.
     */
    ...
    /*
     * @param dateString a date string in one of the formats defined in Netscape cookie spec
     *
     * @return delta seconds between this cookie's creation time and the time specified by
     * dateString
     */
