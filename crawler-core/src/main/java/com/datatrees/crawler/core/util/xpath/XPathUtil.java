...
public class XPathUtil {
  ...
  private static Collection<? extends String> useXpathSelect(String xpath, String content,
      boolean extractText, String split) {
    List<String> splits = new LinkedList<String>();
    try {
          ...
          if (obj instanceof TagNode) {
            ...
            if (extractText) {
              if (StringUtils.isEmpty(split)) {
                splits.add(com.datatrees.common.util.StringUtils
                    .replaceBlank(resultNode.getText().toString()));
              } else {
                splits.add(
                    com.datatrees.common.util.StringUtils.replaceBlank(getText(resultNode, split)));
              }
            } else {
  ...
  static final class CustomProcessor extends AbstractXMLOutputProcessor {
    ...
    @Override
    public void process(final Writer out, final Format format, final Element element)
        throws IOException {
