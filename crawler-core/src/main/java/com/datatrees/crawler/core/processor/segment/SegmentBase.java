...
public abstract class SegmentBase<T extends AbstractSegment> extends Processor {
  ...
  private void processExtractor(Request request, Response response) throws Exception {
    ...
    for (String split : splits) {
      if (StringUtils.isNotBlank(segment.getBreakPattern())
          && StringUtils.isNotBlank(
          breakPattern = SourceUtil.sourceExpression(request, response, segment.getBreakPattern()))
          && PatternUtils.match(breakPattern, split,
          segment.getBreakPatternFlag() != null ? segment.getBreakPatternFlag() : 0)) {
        logger.warn(segment + " match the break pattern:" + breakPattern + " break...");
        break;
      }
      if (StringUtils.isNotBlank(segment.getDisContains())
          && StringUtils.isNotBlank(
          disContains = SourceUtil.sourceExpression(request, response, segment.getDisContains()))
          && PatternUtils.match(disContains, split,
          segment.getDisContainsFlag() != null ? segment.getDisContainsFlag() : 0)) {
        logger.info("split filtered,matches the dis-contains pattern:" + disContains);
        continue;
      }
      if (StringUtils.isNotBlank(segment.getContains())
          && StringUtils.isNotBlank(
          contains = SourceUtil.sourceExpression(request, response, segment.getContains()))
          && !PatternUtils.match(contains, split,
          segment.getContainsFlag() != null ? segment.getContainsFlag() : 0)) {
      ...
      for (AbstractSegment abstractSegment : segments) {
        ...
        try {
          if (BooleanUtils.isTrue(abstractSegment.getStandBy())
              && resultMap.get(abstractSegment.getName()) != null
              && isValid(resultMap.get(abstractSegment.getName()))) {
          ...
          if (StringUtils.isEmpty(content)) {
            ...
            RequestUtil
                .setRequestVisibleFields(newRequest, RequestUtil.getRequestVisibleFields(request));
        ...
        if (BooleanUtils.isTrue(abstractSegment.getNotEmpty())
            && (segResultList == null || (segResultList instanceof Collection && CollectionUtils
            .isEmpty((Collection) segResultList)))) {
      ...
      if (CollectionUtils.isEmpty(segment.getSegmentList())
          && ResultType.getResultType(segment.getResultClass()) != null) {
  ...
  private void segmentsResultsConvert(Request request, Response response) {
    ...
    String baseURL = null;
    if (current != null) {
      baseURL = (StringUtils.isNotEmpty(current.getBaseUrl()) ? current.getBaseUrl()
          : current.getUrl());
    }
  ...
  private void valueListToResultMap(Map<String, Object> resultMap, Object segResultList,
      String segmentName) {
   ...
   */
  @SuppressWarnings({"rawtypes", "unchecked"})
  private void fieldWrapperMapToField(Map<String, Object> originalResultMap,
      Map<String, FieldExtractorWarpper> fieldMap) {
