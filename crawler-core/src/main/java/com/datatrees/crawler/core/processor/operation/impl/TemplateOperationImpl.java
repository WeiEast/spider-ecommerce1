...
public class TemplateOperationImpl extends Operation {
    ...
    public void process(Request request, Response response) throws Exception {
        ...
        if (BooleanUtils.isTrue(op.getReturnObject())) {
            output =
                    ReplaceUtils.getReplaceObject(replaceList, FieldExtractorWarpperUtil.fieldWrapperMapToField(fieldMap),
                            RequestUtil.getSourceMap(request), template);
        } else {
            output =
                    ReplaceUtils.replaceMap(replaceList, FieldExtractorWarpperUtil.fieldWrapperMapToField(fieldMap),
                            RequestUtil.getSourceMap(request), template);
        }
