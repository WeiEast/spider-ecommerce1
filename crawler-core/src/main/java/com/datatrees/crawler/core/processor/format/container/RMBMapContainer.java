...
public class RMBMapContainer {
    ...
    private void initMap(String config, Map<String, RMBUnit> rmbMapper) {
        if (StringUtils.isNotEmpty(config)) {
            try {
                @SuppressWarnings("unchecked") Map<String, String> rmbMap = (LinkedHashMap<String, String>) GsonUtils.fromJson(config, new TypeToken<LinkedHashMap<String, String>>() {}.getType());
                ListIterator<Map.Entry<String, String>> i = new ArrayList<Map.Entry<String, String>>(rmbMap.entrySet()).listIterator(rmbMap.size());
                while (i.hasPrevious()) {
