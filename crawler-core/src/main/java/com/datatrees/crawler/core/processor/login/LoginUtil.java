...
public class LoginUtil {
  ...
  public String doLogin(LoginConfig config, SearchProcessorContext context) {
    try {
      ...
      if (StringUtils.isNotEmpty(jsonHeader)) {
        Map<String, String> headersMap = (Map<String, String>) GsonUtils
            .fromJson(jsonHeader, Map.class);
        linkNode.addHeaders(headersMap);
      }
      // get retainQuote
      boolean retainQuote =
          context.getCookieConf() != null ? context.getCookieConf().getRetainQuote() : false;
   ...
   */
  @SuppressWarnings("unchecked")
  public boolean doLoginByCookies(LoginConfig config, SearchProcessorContext context)
      throws ResultEmptyException {
    try {
      LoginCheckConfig loginCheckConfig = config.getLoginCheckConfig();

      String checkUrl = ReplaceUtils
          .replaceMap(context.getContext(), loginCheckConfig.getCheckUrl());
      ...
      logger.error(
          "LoginUtil doLoginByCookies throw exception, error message:[" + ex.getMessage() + "]",
          ex);
    }
    return false;
  }
  ...
  private boolean checkLoginResult(ProtocolOutput out, LoginCheckConfig loginCheckConfig,
    ...
    if (out != null && out.getContent() != null) {
      ...
      String successPattern = ReplaceUtils
          .replaceMap(context.getContext(), loginCheckConfig.getSuccessPattern());
  ...
  private void callSegments(List<AbstractSegment> segments, SearchProcessorContext context,
      String responseBody) throws ResultEmptyException {
  ...
  private boolean isSuccess(String responseBody, SearchProcessorContext context,
