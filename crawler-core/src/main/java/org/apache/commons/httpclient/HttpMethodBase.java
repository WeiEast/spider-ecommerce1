 ...
 * /java/org/apache/commons/httpclient/HttpMethodBase.java,v 1.222 2005/01/14 21:16:40 olegk Exp $
 * $Revision: 345322 $ $Date: 2005-11-17 15:20:08 -0500 (Thu, 17 Nov 2005) $
 * 
 * ====================================================================
 * 
...
public abstract class HttpMethodBase implements HttpMethod {
    ...
    /**
     * Obtains the name of the HTTP method as used in the HTTP request line, for example
     * <tt>"GET"</tt> or <tt>"POST"</tt>.
     * 
    ...
    /**
     * Sets whether or not the HTTP method should automatically follow HTTP redirects (status code
     * 302, etc.)
     *
    ...
    /**
     * Returns <tt>true</tt> if the HTTP method should automatically follow HTTP redirects (status
     * code 302, etc.), <tt>false</tt> otherwise.
     *
    ...
    /**
     * Sets whether or not the HTTP method should automatically handle HTTP authentication
     * challenges (status code 401, etc.)
     *
    ...
    /**
     * Sets the path of the HTTP method. It is responsibility of the caller to ensure that the path
     * is properly encoded (URL safe).
     *
     ...
     * executed will return the <em>actual</em> path, following any redirects automatically handled
     * by this HTTP method.
     *
    ...
    /**
     * Set the specified request header, overwriting any previous value. Note that header-name
     * matching is case-insensitive.
     *
    ...
    /**
     * Sets the specified request header, overwriting any previous value. Note that header-name
     * matching is case insensitive.
     *
    ...
    /**
     * Gets the {@link HeaderGroup header group} storing the response trailer headers as per RFC
     * 2616 section 3.6.1.
     *
     * @return a HeaderGroup
     *
    ...
    /**
     * Returns an array of the response headers that the HTTP method currently has in the order in
     ...
     * insensitive. <tt>null</tt> will be returned if either <i>headerName</i> is <tt>null</tt> or
     * there is no matching header for <i>headerName</i>.
     *
     ...
     * </p>
     *
     * @return content length, if <tt>Content-Length</tt> header is available. <tt>0</tt> indicates
     *         that the request has no body. If <tt>Content-Length</tt> header is not present, the
     *         method returns <tt>-1</tt>.
     */
    ...
    /**
     * Returns the response body of the HTTP method, if any, as an array of bytes. If response body
     * is not available or cannot be read, returns <tt>null</tt>
     * 
     * Note: This will cause the entire response body to be buffered in memory. A malicious server
     * may easily exhaust all the VM memory. It is strongly recommended, to use getResponseAsStream
    ...
    /**
     * Returns the response body of the HTTP method, if any, as an {@link InputStream}. If response
     * body is not available, returns <tt>null</tt>
     * 
     * @return The response body
     * 
     * @throws IOException If an I/O (transport) problem occurs while obtaining the response body.
    ...
    /**
     * Returns the response body of the HTTP method, if any, as a {@link String}. If response body
     * is not available or cannot be read, returns <tt>null</tt> The string conversion on the data
     * is done using the character encoding specified in <tt>Content-Type</tt> header.
     * 
     * Note: This will cause the entire response body to be buffered in memory. A malicious server
     * may easily exhaust all the VM memory. It is strongly recommended, to use getResponseAsStream
     * if the content length of the response is unknown or resonably large.
    ...
    /**
     * Returns an array of the response footers that the HTTP method currently has in the order in
     * which they were read.
     *
     ...
     * insensitive. <tt>null</tt> will be returned if either <i>footerName</i> is <tt>null</tt> or
     * there is no matching footer for <i>footerName</i> or there are no footers available. If there
     * are multiple footers with the same name, there values will be combined with the ',' separator
    ...
    /**
     * Returns a stream from which the body of the current response may be read. If the method has
     * not yet been executed, if <code>responseBodyConsumed</code> has been called, or if the stream
     ...
     * specification, whereas in non-strict mode it attempts to mimic the exact behaviour of
     * commonly used HTTP agents, which many HTTP servers expect.
     * 
    ...
    /**
     * Tests if the connection should be closed after the method has been executed. The connection
     * will be left open when using HTTP/1.1 or if <tt>Connection: 
     ...
     *        non-null.
     * @param conn the {@link HttpConnection connection} to used to execute this HTTP method. Must
     *        be non-null.
     *
    ...
    /**
     * Recycles the HTTP method so that it can be used again. Note that all of the instance
     * variables will be reset once this method has been called. This method will also release the
     * connection being used by this HTTP method.
     * 
    ...
    /**
     * Releases the connection being used by this HTTP method. In particular the connection is used
     * to read the response(if there is one) and will be held until the response has been read. If
     * the connection can be reused by other HTTP methods it is NOT closed at this point.
     *
     * @since 2.0
     */
    ...
    /**
     * Remove the request header associated with the given name. Note that header-name matching is
     * case insensitive.
     *
    ...
    /**
     * Generates <tt>Cookie</tt> request headers for those {@link Cookie cookie}s that match the
    ...
    protected void addDistinctCookieRequestHeader(HttpState state, HttpConnection conn) throws IOException, HttpException {
        ...
        if ((cookies != null) && (cookies.length > 0)) {
            ...
            for (int i = 0; i < cookieheaders.length; i++) {
                ...
                // continue;
                // }
                cookieMap.putAll(CookieFormater.INSTANCE.parserCookieToMap(cookieheader.getValue(), retainQuote));
            }
            String s = matcher.formatCookies(cookies);
            Map<String, String> newMap = CookieFormater.INSTANCE.parserCookieToMap(s, retainQuote);
            cookieMap.putAll(newMap);
            if (getParams().isParameterTrue(HttpMethodParams.SINGLE_COOKIE_HEADER)) {
                // In strict mode put all cookies on the same header
                getRequestHeaderGroup().addHeader(new Header("Cookie", CookieFormater.INSTANCE.listToString(cookieMap), true));
            } else {
                // In non-strict mode put each cookie on a separate header
                String[] cookieArray = CookieFormater.INSTANCE.parserCookieToArray(CookieFormater.INSTANCE.listToString(cookieMap), retainQuote);
                for (int i = 0; i < cookieArray.length; i++) {
                    getRequestHeaderGroup().addHeader(new Header("Cookie", cookieArray[i], true));
                }
            }
        }
    }
     ...
     * <tt>Authorization</tt>, <tt>Proxy-Authorization</tt> and <tt>Proxy-Connection</tt> headers,
     * when appropriate.
     * </p>
     *
     * <p>
    ...
    /**
     * Generates default <tt>User-Agent</tt> request header, as long as no <tt>User-Agent</tt>
    ...
    /**
     * Throws an {@link IllegalStateException} if the HTTP method has not been {@link #execute
     * executed} since last {@link #recycle recycle}.
     *
     *
    ...
    /**
     * This method is invoked immediately after {@link #readResponseBody(HttpState, HttpConnection)}
     ...
     * {@link #readResponseHeaders(HttpState, HttpConnection)} and can be overridden by sub-classes
     * in order to provide custom response headers processing.
     * 
     * <p>
     * This implementation will handle the <tt>Set-Cookie</tt> and <tt>Set-Cookie2</tt> headers, if
    ...
    /**
     * This method is invoked immediately after {@link #readStatusLine(HttpState, HttpConnection)}
     * and can be overridden by sub-classes in order to provide custom response status line
     * processing.
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
     ...
     * <ol>
     * <li>
     * {@link #readStatusLine(HttpState, HttpConnection)} is invoked to read the request line.</li>
     * <li>
     * {@link #processStatusLine(HttpState, HttpConnection)} is invoked, allowing the method to
     * process the status line if desired.</li>
     * <li>
     * {@link #readResponseHeaders(HttpState, HttpConnection)} is invoked to read the associated
     * headers.</li>
     * <li>
     * {@link #processResponseHeaders(HttpState, HttpConnection)} is invoked, allowing the method to
     * process the headers if desired.</li>
     * <li>
     * {@link #readResponseBody(HttpState, HttpConnection)} is invoked to read the associated body
     * (if any).</li>
     * <li>
     * {@link #processResponseBody(HttpState, HttpConnection)} is invoked, allowing the method to
     * process the response body if desired.</li>
     * </ol>
     *
     * Subclasses may want to override one or more of the above methods to to customize the
     * processing. (Or they may choose to override this method if dramatically different processing
     * is required.)
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     ...
     *
     * <p>
     * The current implementation wraps the socket level stream with an appropriate stream for the
     * type of response (chunked, content-length, or auto-close). If there is no response body, the
    ...
    /**
     * Returns the response body as an {@link InputStream input stream} corresponding to the values
     * of the <tt>Content-Length</tt> and <tt>Transfer-Encoding</tt> headers. If no response body is
     ...
     * to the first, each separated by a comma." - HTTP/1.0 (4.3)
     * </p>
     *
     * @param state the {@link HttpState state} information associated with this method
     * @param conn the {@link HttpConnection connection} used to execute this HTTP method
     ...
     * <ol>
     * <li>
     * {@link #writeRequestLine(HttpState, HttpConnection)} is invoked to write the request line.</li>
     * <li>
     * {@link #writeRequestHeaders(HttpState, HttpConnection)} is invoked to write the associated
     * headers.</li>
     * <li>
     * <tt>\r\n</tt> is sent to close the head part of the request.</li>
     * <li>
     * {@link #writeRequestBody(HttpState, HttpConnection)} is invoked to write the body part of the
     * request.</li>
     * </ol>
     *
     * <p>
     ...
     *
     * <p>
     * This method should return <tt>true</tt> if the request body was actually sent (or is empty),
     * or <tt>false</tt> if it could not be sent for some reason.
     * </p>
     *
     * <p>
     ...
     *
     * <p>
     * This implementation invokes {@link #addRequestHeaders(HttpState, HttpConnection)}, and then
     * writes each header to the request stream.
     * </p>
     *
     * <p>
     * Subclasses may want to override this method to to customize the processing.
    ...
    /**
     * Returns the HTTP version used with this method (may be <tt>null</tt> if undefined, that is,
     ...
     *
     * @return <tt>true</tt> if the message may contain a body, <tt>false</tt> if it can not contain
     *         a message body
     */
     ...
     *
     * <p>
     * The default behavior for this class is to check to see if the connection should be closed,
     * and close if need be, and to ensure that the connection is returned to the connection manager
     * - if and only if we are not still inside the execute call.
     * </p>
     *
     */
    ...
    /**
     * This method is a dirty hack intended to work around current (2.0) design flaw that prevents
     * the user from obtaining correct status code, headers and response body from the preceding
     * HTTP CONNECT method.
     * 
     ...
     * 
     * @return <tt>true</tt> if the execution of this method has been aborted, <tt>false</tt>
     *         otherwise
     * 
     * @since 3.0
     */
     ...
     * <tt>false</tt> otherwise. This flag can be useful for recovery logic. If the request has not
     * been transmitted in its entirety, it is safe to retry the failed method.
     * 
