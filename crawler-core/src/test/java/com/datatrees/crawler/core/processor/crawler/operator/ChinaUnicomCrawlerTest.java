...
import com.datatrees.crawler.core.processor.bean.*;
...
public class ChinaUnicomCrawlerTest extends BaseConfigTest {
    ...
    public void testLinkNodesDiscoverSearchTemplatesCrawlerAndExtract() throws ResultEmptyException {
        ...
        try {
            ...
            ExtractorProcessorContext extractorContext = getExtractorProcessorContext("operator/chinaunicom-extractor.xml", "10010.com");

            /**
             * 本用例没有使用登陆产生cookie，而是直接写入以下cookie，失效后请更换
             */
            ProcessorContextUtil.setCookieString(context, "BIGipServerPOOL_WSYYT2_4GqiantaiAPP=789255178.16671.0000; e3=JLmjVwNWr2w0G5TCThN8qfNBvHyyc4yl4TvBkvwQxhQHKCg1vfsH!446242911!807301151;");
            ...
            CrawlRequest request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode("http://iservice.10010.com/e3/query/call_dan.html")).setSearchTemplateId("calldetails-linknodes-discover-search-template").contextInit();
            CrawlResponse resp = Crawler.crawl(request);
            logger.info("calldetails linknodes discover search response status - " + ResponseUtil.getResponseStatus(resp));
            ...
            logger.info("calldetails linknodes discover search link nodes - " + ResponseUtil.getResponseLinkNodes(resp));
            ...
            if (CollectionUtils.isNotEmpty(ResponseUtil.getResponseLinkNodes(resp))) {
                for (LinkNode linkNode : ResponseUtil.getResponseLinkNodes(resp)) {
                    ...
                    CrawlRequest subRequest = CrawlRequest.build().setProcessorContext(context).setUrl(linkNode).setSearchTemplateId("calldetails-search-template").contextInit();
                    ...
                    if (Status.BLOCKED == (int) ResponseUtil.getResponseStatus(subResp)) {
                        ...
                        if (CollectionUtils.isEmpty(objs)) {
                            logger.warn("response - " + ResponseUtil.getProtocolResponse(subResp).getContent().getContentAsString());
                        } else {
                            // 保存 做处理 。。。
                            for (Object obj : objs) {
                            ...
                            ExtractorRepuest extractorRequest = ExtractorRepuest.build().setProcessorContext(extractorContext);
                            extractorRequest.setInput((Map) objs.get(0));
                            Response extractorResponse = Extractor.extract(extractorRequest);

                            List<Object> extractorResultObjectList = ResponseUtil.getResponseObjectList(extractorResponse);
                            logger.info("calldetails extractor response resultObjectList - {}", extractorResultObjectList);
                        }
                    }
                }
            }
            ...
            request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode("http://iservice.10010.com/e3/query/call_sms.html")).setSearchTemplateId("smsdetails-linknodes-discover-search-template").contextInit();
            resp = Crawler.crawl(request);
            logger.info("smsdetails linknodes discover search response status - " + ResponseUtil.getResponseStatus(resp));
            Assert.assertTrue((ResponseUtil.getResponseStatus(resp) == ProtocolStatusCodes.SUCCESS || ResponseUtil.getResponseStatus(resp) == Status.NO_SEARCH_RESULT));
            ...
            if (CollectionUtils.isNotEmpty(ResponseUtil.getResponseLinkNodes(resp))) {
                for (LinkNode linkNode : ResponseUtil.getResponseLinkNodes(resp)) {
                    logger.info(linkNode + "  propertys " + linkNode.getPropertys());
                    logger.info("Crawler url " + linkNode.getUrl());
                    CrawlRequest subRequest = CrawlRequest.build().setProcessorContext(context).setUrl(linkNode).setSearchTemplateId("smsdetails-search-template").contextInit();
                    CrawlResponse subResp = Crawler.crawl(subRequest);
                    ...
                    if (Status.BLOCKED == (int) ResponseUtil.getResponseStatus(subResp)) {
                        ...
                        if (CollectionUtils.isEmpty(objs)) {
                            ...
                            List<Object> extractorResultObjectList = ResponseUtil.getResponseObjectList(extractorResponse);
                            logger.info("smsdetails extractor response resultObjectList - {}", extractorResultObjectList);
                        }
                    }
                }
            }
             ...
             */
            start = System.currentTimeMillis();
            request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode("http://iservice.10010.com/e3/query/history_list.html")).setSearchTemplateId("billdetails-linknodes-discover-search-template").contextInit();
            resp = Crawler.crawl(request);
            logger.info("billdetails linknodes discover search response status - " + ResponseUtil.getResponseStatus(resp));
            Assert.assertTrue((ResponseUtil.getResponseStatus(resp) == ProtocolStatusCodes.SUCCESS || ResponseUtil.getResponseStatus(resp) == Status.NO_SEARCH_RESULT));
            logger.info("billdetails linknodes discover search link nodes - " + ResponseUtil.getResponseLinkNodes(resp));
            ...
            if (CollectionUtils.isNotEmpty(ResponseUtil.getResponseLinkNodes(resp))) {
                for (LinkNode linkNode : ResponseUtil.getResponseLinkNodes(resp)) {
                    logger.info(linkNode + "  propertys " + linkNode.getPropertys());
                    logger.info("Crawler url " + linkNode.getUrl());
                    CrawlRequest subRequest = CrawlRequest.build().setProcessorContext(context).setUrl(linkNode).setSearchTemplateId("billdetail-search-template").contextInit();
                    CrawlResponse subResp = Crawler.crawl(subRequest);
                    ...
                    if (Status.BLOCKED == (int) ResponseUtil.getResponseStatus(subResp)) {
                        ...
                        if (CollectionUtils.isEmpty(objs)) {
                            ...
                            List<Object> extractorResultObjectList = ResponseUtil.getResponseObjectList(extractorResponse);
                            logger.info("billdetails extractor response resultObjectList - {}", extractorResultObjectList);
                        }
                    }
                }
            }
    ...
    public void testSeparateSearchTemplates() throws ResultEmptyException {
        ...
        try {
             ...
             * 本用例没有使用登陆产生cookie，而是直接写入以下cookie，失效后请更换
             */
            ProcessorContextUtil.setCookieString(context, "BIGipServerPOOL_WSYYT2_4GqiantaiAPP=755700746.17695.0000; e3=zwwfVwPPCGMTwNq1d8VwyVtqDH21VTKn42lkqJH3QTJRbJBFs1dM!2141628305!-148112031;");

            /**
             * personal information search
             */
            long start = System.currentTimeMillis();
            CrawlRequest request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode("http://iservice.10010.com/e3/static/query/accountBalance/search\"")).setSearchTemplateId("personalinformation-search-template").contextInit();
            CrawlResponse resp = Crawler.crawl(request);
             ...
             */
            start = System.currentTimeMillis();
            request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode("http://iservice.10010.com/e3/static/query/currentFee\"")).setSearchTemplateId("currentfee-search-template").contextInit();
            resp = Crawler.crawl(request);
            ...
            while (requestedMonths-- > 0) {
                ...
                urlBuilder.append("http://iservice.10010.com/e3/static/query/callDetail").append("\"").append("pageNo=").append(1).append("&pageSize=").append(Integer.MAX_VALUE).append("&beginDate=").append(new LocalDate().minusMonths(monthIndex).dayOfMonth().withMinimumValue().toString("yyyy-MM-dd")).append("&endDate=").append(new LocalDate().minusMonths(monthIndex).dayOfMonth().withMaximumValue().toString("yyyy-MM-dd"));
                request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode(urlBuilder.toString())).setSearchTemplateId("calldetails-search-template").contextInit();
                resp = Crawler.crawl(request);
            ...
            while (requestedMonths-- > 0) {
                start = System.currentTimeMillis();
                urlBuilder = new StringBuilder();
                urlBuilder.append("http://iservice.10010.com/e3/static/query/sms").append("\"").append("pageNo=").append(1).append("&pageSize=").append(Integer.MAX_VALUE);
                ...
                urlBuilder.append("&begindate=").append(startDate.toString("yyyyMMdd")).append("&enddate=").append(endDate.toString("yyyyMMdd"));
                ...
                request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode(urlBuilder.toString()).addHeaders(headersMap)).setSearchTemplateId("smsdetails-search-template").contextInit();
                resp = Crawler.crawl(request);
            ...
            while (requestedMonths-- > 0) {
                ...
                urlBuilder.append("http://iservice.10010.com/e3/static/query/queryHistoryBill").append("\"")
                        ...
                        .append("billdate=").append(new LocalDate().minusMonths(monthIndex).dayOfMonth().withMinimumValue().toString("yyyyMM"));
                request = CrawlRequest.build().setProcessorContext(context).setUrl(new LinkNode(urlBuilder.toString())).setSearchTemplateId("billdetail-search-template").contextInit();
                resp = Crawler.crawl(request);
