...
public class MailServiceApiForSinaImpl implements MailServiceApiForSina {
    ...
    private static final Pattern PRELOGIN_RESULT_PATTERN =
            PatternUtils
                    .compile("\"retcode\":(\\d+),\"servertime\":(\\d+),\"pcid\":\"([^\"]+)\",\"nonce\":\"([^\"]+)\",\"pubkey\":\"([^\"]+)\",\"rsakv\":\"(\\d+)\"");
    ...
    @Override
    public HttpResult<Map<String, Object>> loginInit(CommonPluginParam param) {
        ...
        try {
            ...
            response = TaskHttpClient.create(param.getTaskId(), param.getWebsiteName(), RequestType.GET, "sina_com_cn_01").setFullUrl(MAIN_URL).invoke();
            ...
            response = TaskHttpClient.create(param.getTaskId(), param.getWebsiteName(), RequestType.GET, "sina_com_cn_02").setFullUrl(preLoginUrl).invoke();
    ...
    @Override
    public HttpResult<Map<String, Object>> login(CommonPluginParam param) {
        ...
        try {
            ...
            if (param.getPicCode() == null) {
                templateData = "entry=cnmail&gateway=1&from=&savestate=30&qrcode_flag=false&useticket=0&pagerefer=&cw=1&su={}&service=sso&servertime={}&nonce={}&pwencode=rsa2&rsakv={}&sp={}&sr=1920*1200&encoding=UTF-8&cdult=3&domain=sina.com.cn&prelt=46&returntype=TEXT";
                data = TemplateUtils.format(templateData, su, serverTime, nonce, rsakv, sp);
            } else {
                templateData = "entry=cnmail&gateway=1&from=&savestate=30&qrcode_flag=false&useticket=0&pagerefer=&cw=1&pcid={}&door={}&su={}&service=sso&servertime={}&nonce={}&pwencode=rsa2&rsakv={}&sp={}&sr=1920*1200&encoding=UTF-8&cdult=3&domain=sina.com.cn&prelt=46&returntype=TEXT";
                data = TemplateUtils.format(templateData, pcId, param.getPicCode(), su, serverTime, nonce, rsakv, sp);
            }
            response = TaskHttpClient.create(param.getTaskId(), param.getWebsiteName(), RequestType.POST, "").setFullUrl(requestUrl).setRequestBody(data).invoke();
            String pageContent = response.getPageContent();
            ...
            if (pageContent.contains("101")) {
                ...
                for (String crossUrl : urlList) {
                    crossUrl = crossUrl + "&callback=sinaSSOController.doCrossDomainCallBack&scriptId=ssoscript" + i + "&client=ssologin.js(v1.4.19)&_=" + System.currentTimeMillis();
                ...
                messageService.sendMessage(TopicEnum.RAWDATA_INPUT.getCode(), TopicTag.LOGIN_INFO.getTag(), mqMap, DEFAULT_CHARSET_NAME);
                return result.success(map);
    ...
    @Override
    public HttpResult<Map<String, Object>> refeshPicCode(CommonPluginParam param) {
        if (param.getTaskId() == null) {
            throw new RuntimeException(ErrorCode.PARAM_ERROR.getErrorMsg());
        }
        param.setWebsiteName(GroupEnum.MAIL_SINA_H5.getWebsiteName());
