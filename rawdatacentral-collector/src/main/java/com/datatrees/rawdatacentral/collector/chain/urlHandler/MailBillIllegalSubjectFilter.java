...
public class MailBillIllegalSubjectFilter extends RemovedFetchLinkNodeFilter {
    ...
    private static Pattern subjectContainsPattern        = Pattern
            .compile(PropertiesConfiguration.getInstance().get("mail.subject.contains.pattern", "账单|信用卡|对账"));
    ...
    private static Pattern secondSubjectBlackListPattern = Pattern
            .compile(PropertiesConfiguration.getInstance().get("mail.subject.second.blacklist.pattern", "分期感恩季"));

    private static Pattern subjectWhiteListPattern       = Pattern
            .compile(PropertiesConfiguration.getInstance().get("mail.subject.whiltelist.pattern", "招商"));
        ...
        if (subject != null && websiteType != null && WebsiteType.MAIL.getValue().equals(websiteType) &&
            ...
            if (PatternUtils.match(subjectContainsPattern, subject.toString().replaceAll("\\s", ""))) {
                if (PatternUtils.match(subjectBlackListPattern, subject.toString())) {
                    if (!PatternUtils.match(subjectWhiteListPattern, subject.toString()) ||
