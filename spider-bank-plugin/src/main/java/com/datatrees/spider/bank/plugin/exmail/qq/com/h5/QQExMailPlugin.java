...
package com.datatrees.spider.bank.plugin.exmail.qq.com.h5;
...
import com.datatrees.spider.bank.plugin.exmail.qq.com.h5.util.EncryptExMailQQUtils;
import com.datatrees.spider.bank.plugin.exmail.qq.com.h5.util.ExMailErrorEnum;
 ...
 */
public class QQExMailPlugin implements CommonPlugin {
    ...
    public HttpResult<Object> submit(CommonPluginParam param) {
        ...
        try {
            ...
            if (pageContent.contains("正在登录腾讯企业邮箱") && pageContent.contains("\"frame_html?sid=")) {
                ...
                String currentUrl;
                if (pageContent.contains("正在登录腾讯企业邮箱") && pageContent.contains("var target=\"ERROR\";") &&
                        pageContent.contains("urlHead + \"loginpage?")) {
                    String urlHead = PatternUtils.group(pageContent, URLHEAD_RESULT_PATTERN, 1);
    ...
    private HttpResult<Object> failForResult(CommonPluginParam param, HttpResult<Object> result, MonitorService monitorService,
            Map<String, Object> map, Long taskId, String errorString) {
        MessageService messageService = BeanFactoryUtils.getBean(MessageService.class);
